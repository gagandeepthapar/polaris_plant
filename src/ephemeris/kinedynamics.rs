...
pub fn orbital_twobody(_t: f64, state0: &Generic1D, _inpt: &Generic2D) -> Generic1D {
    /*
    Inputs:
    ...
    */
    ...
    // Compute Accel
    let (_, mrsc) = unit(rsc.to_owned().insert_axis(Axis(1)));
    let asc = -1. * consts::MU * rsc.to_owned() / (mrsc.powi(3));
